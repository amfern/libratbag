...
cmds+=("profile 0 led logo get")
cmds+=("profile 0 led side get")
cmds+=("profile 0 led logo set mode on")
cmds+=("profile 0 led logo set color ffffff")
cmds+=("profile 0 led logo set rate 1")
cmds+=("profile 0 led logo set brightness 1")
for i in "${cmds[@]}"; do
	echo "Testing arguments '$i $device'"
